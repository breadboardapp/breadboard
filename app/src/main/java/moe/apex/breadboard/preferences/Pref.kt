...
import moe.apex.breadboard.image.Gelbooru
...
import moe.apex.breadboard.image.Rule34
import moe.apex.breadboard.image.Safebooru
...
class UserPreferencesRepository(private val dataStore: DataStore<Preferences>) {
    ...
    suspend fun handleMigration(context: Context) {
        ...
        /* Version 3.2.0 fixes HTML-encoded tags in favourites. */
        if (lastUsedVersionCode < 320) {
            val data = dataStore.data.first()
            val favImagesByteArray = data[PreferenceKeys.FAVOURITE_IMAGES]
            if (favImagesByteArray != null) {
                val favImages: List<Image> = Cbor.decodeFromByteArray(favImagesByteArray)
